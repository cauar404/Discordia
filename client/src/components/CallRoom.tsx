...
export function CallRoom({ kind, onLeave, isMinimized = false, onMinimize, onRestore, voiceVideoSettings, onVoiceVideoSettingsChange, microphoneToggleSignal, onMicrophoneStateChange }: { kind: CallKind; onLeave: () => void | Promise<void>; isMinimized?: boolean; onMinimize?: () => void; onRestore?: () => void; voiceVideoSettings?: VoiceVideoSettings; onVoiceVideoSettingsChange?: (settings: VoiceVideoSettings) => void | Promise<unknown>; microphoneToggleSignal?: number; onMicrophoneStateChange?: (enabled: boolean) => void }) {
  ...
  return <section className={cn("call-room", "call-room-polished", screenShares.length > 0 && "has-screen-share", focusedScreenShare && "has-focused-share", isMinimized && "is-minimized")} aria-label="Sala de chamada">
    ...
    <section className="call-dock" aria-label="Controles rápidos da chamada">
      <button type="button" className={cn("call-dock-control", !isMicrophoneEnabled && "is-off", pushToTalkEnabled && "is-active")} onClick={() => void toggleMicrophone()} disabled={pushToTalkEnabled} aria-label={pushToTalkEnabled ? `Aperte ${pushToTalkKeyLabel(pushToTalkKey)} para falar` : isMicrophoneEnabled ? "Desativar microfone" : "Ativar microfone"} title={pushToTalkEnabled ? `Aperte ${pushToTalkKeyLabel(pushToTalkKey)} para falar` : isMicrophoneEnabled ? "Desativar microfone" : "Ativar microfone"}>{pushToTalkEnabled || isMicrophoneEnabled ? <Mic className="size-5" /> : <MicOff className="size-5" />}<span>{pushToTalkEnabled ? (isPushToTalkPressed ? "Falando" : "Aperte para falar") : "Microfone"}</span></button>
      <button type="button" className={cn("call-dock-control", !isCameraEnabled && "is-off")} onClick={() => void toggleCamera()} aria-label={isCameraEnabled ? "Desativar câmera" : "Ativar câmera"} title={isCameraEnabled ? "Desativar câmera" : "Ativar câmera"}>{isCameraEnabled ? <Video className="size-5" /> : <VideoOff className="size-5" />}<span>Câmera</span></button>
      <button type="button" className={cn("call-dock-control", isScreenShareEnabled && "is-active")} onClick={() => void toggleScreenShare()} disabled={isUpdatingShare} aria-label={isScreenShareEnabled ? "Parar compartilhamento" : "Compartilhar tela"} title={isScreenShareEnabled ? "Parar compartilhamento" : "Compartilhar tela"}><ScreenShare className="size-5" /><span>{isScreenShareEnabled ? "Parar tela" : "Compartilhar"}</span></button>
      <button type="button" className="call-dock-end" onClick={() => void onLeave()} aria-label="Sair da chamada" title="Sair da chamada"><PhoneOff className="size-5" /></button>
    </section>
    <details className="call-settings-panel">
      <summary><span><Settings2 className="size-4" /> Configurações da chamada <small>Áudio, tela e conexão</small></span><ChevronDown className="size-4" /></summary>
