...
export default function Home() {
  ...
  const redeemInvite = trpc.platform.communities.redeemInvite.useMutation({ onSuccess: async () => { localStorage.removeItem("circulo-invite-code"); await utils.platform.profile.me.invalidate(); await utils.platform.communities.list.invalidate(); toast.success("Convite aceito. Seu acesso foi liberado."); }, onError: error => { localStorage.removeItem("circulo-invite-code"); toast.error(error.message); } });
  ...
  useEffect(() => {
    ...
    const socket = io({ path: "/api/realtime", transports: ["websocket"], withCredentials: true });
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (editingMessageId) { if (draft.trim()) await updateMessage.mutateAsync({ messageId: editingMessageId, content: draft.trim() }); return; } if (channelId && (draft.trim() || pendingFiles.length) && activeChannel?.type !== "voice") { const files = await Promise.all(pendingFiles.map(async file => ({ fileName: file.name, mimeType: file.type || "application/octet-stream", base64: await encodeFile(file) }))); await send.mutateAsync({ channelId, content: draft.trim(), replyToMessageId, files }); setReplyToMessageId(null); } }
  ...
  async function beginCall(kind: "voice" | "video") {
    ...
    try { const created = await startCall.mutateAsync({ kind, channelId }); const credentials = await joinCall.mutateAsync({ callId: created.callId }); setCall({ callId: credentials.call.id, serverUrl: credentials.serverUrl, token: credentials.token, kind: credentials.call.kind }); await utils.social.calls.active.invalidate(); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível iniciar a chamada."); }
  }
  async function closeCall() { const currentCall = call; if (!currentCall || callClosingRef.current === currentCall.callId) return; callClosingRef.current = currentCall.callId; setCall(null); try { await leaveCall.mutateAsync({ callId: currentCall.callId }); await utils.social.calls.active.invalidate(); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível registrar sua saída da chamada."); } finally { callClosingRef.current = null; } }
  ...
  return <main className="app-shell">
    ...
    {call && <div className="call-overlay"><LiveKitRoom serverUrl={call.serverUrl} token={call.token} connect audio video={call.kind === "video"} onDisconnected={() => void closeCall()}><CallRoom kind={call.kind} onLeave={closeCall} voiceVideoSettings={profile.data?.settings?.voiceVideo as Record<string, string | boolean> | undefined} onVoiceVideoSettingsChange={settings => updateSettings.mutateAsync({ voiceVideo: settings })} /></LiveKitRoom></div>}
  </main>;
}
